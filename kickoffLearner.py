...
from rlgym_sim.utils.reward_functions.common_rewards import FaceBallReward
...
def build_rocketsim_env():
    ...
    terminal_conditions = [NoTouchTimeoutCondition(timeout_ticks), term.KickoffWonCondition()]

    reward_fn = CombinedReward.from_zipped(
        (rwd.SpeedTowardBallReward(), 5.0),
        (FaceBallReward(), 0.7),
        (rwd.AirReward(), 0.05),
        (rwd.FirstTouchReward(), 2.0),
        (rwd.WinKickoffReward(), 1.0),
    )
...
if __name__ == "__main__":
    ...
    try:
        latest_checkpoint_dir = "data/checkpoints/kickoff-ppo-run/" + str(max(os.listdir("data/checkpoints/kickoff-ppo-run"), key=lambda d: int(d)))
    except:
    ...
    learner = Learner(build_rocketsim_env,
                      ...
                      load_wandb=False,
                      wandb_run_name="kickoffLearner",
                      checkpoints_save_folder="data/checkpoints/kickoff-ppo-run",
